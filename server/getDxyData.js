...
function getDataFromDxy() {
	superagent
		...
		.then((res) => {
			...
			let $getListByCountryTypeService2true = $('#getListByCountryTypeService2true').html();

			let $fetchRecentStatV2 = $('#fetchRecentStatV2').html();
			// console.log($getAreaStat);

			// 使用正则表达式，unescape()函数转编码,字符串切割或 eval函数将代码运行
			let dataObj = {}; // 定义dataObj对象换掉源码的 window
